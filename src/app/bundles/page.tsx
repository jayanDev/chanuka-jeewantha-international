...
const bundles = internationalBundles.map(bundle => ({
  ...bundle, highlighted: bundle.popular, label: bundle.audience,
  includes: bundle.features, cta: `Choose ${bundle.name}`,
}));
        ...
        <div className="mx-auto grid max-w-[1512px] grid-cols-1 gap-6 px-4 sm:px-6 lg:grid-cols-5">
          {bundles.map((bundle) => (
            ...
            >
              ...
              <p className={`mt-6 font-heading text-[36px] font-bold ${bundle.premium ? "text-white" : "text-foreground"}`}><Price usd={bundle.usd} /></p>
