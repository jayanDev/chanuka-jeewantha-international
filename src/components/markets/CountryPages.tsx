...
function Bundles({ market, preview = false }: { market: Market; preview?: boolean }) {
  ...
  return <div className={styles.bundleGrid}>{bundles.map((bundle) => <article key={bundle.id} className={`${styles.bundle} ${bundle.popular ? styles.popular : ""} ${bundle.premium ? styles.premium : ""}`}><p className={styles.badge}>{bundle.popular ? "Most Popular" : bundle.premium ? "Signature leadership service" : "Founder-led"}</p><h3>{bundle.name}</h3><p>{bundle.audience}</p><p className={styles.bundlePrice}>{marketPrice(bundle.usd, market)}</p><ul className={styles.list}>{bundle.features.map((feature) => <li key={feature}>{feature.replace(/optimi[sz]ation/g, market.optimisation)}</li>)}</ul><Link href={marketEnquiryLink(market, bundle.name)} className={bundle.premium ? styles.goldButton : styles.button}>Request This Package <ArrowUpRight size={16} aria-hidden="true" /></Link></article>)}</div>;
}
...
export function CountryHome({ market }: { market: Market }) {
  ...
  return <>
    ...
    <section className={`${styles.band} ${styles.tint}`}><div className={styles.container}><div className={styles.heading}><p className={styles.eyebrow}>Client feedback</p><h2>Care in the writing. Clarity in the communication.</h2><p>Read client feedback on Google and discuss your target role with Chanuka before choosing your service.</p></div><a className={styles.textLink} href={GOOGLE_REVIEWS_URL} target="_blank" rel="noreferrer">Read client reviews <ArrowUpRight size={16} aria-hidden="true" /></a></div></section>
