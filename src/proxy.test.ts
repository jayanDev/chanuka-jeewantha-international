...
describe("country routing proxy", () => {
  it("matches all country pages without changing valid routes", () => {
  ...
  it.each(["/en-xx", "/en-xx/services", "/en-uk/not-a-page", "/en-uk/blog/not-an-article", "/en-au/services/extra"])("returns a noindex 404 for %s", (path) => {
  ...
  it("preserves existing protected-route authentication", () => {
    const response = proxy(new NextRequest("https://chanukajeewantha.com/profile"));
